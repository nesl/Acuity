...
void process(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &data, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &data_down)
{
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
    {
        ...
        *background_points += *temp_cloud; // Filter on original frame to provide background to subtract
        ...
        //Publish results to the arrOfPeople array, I should really change to standard c++ structs D:
    ...
    if (clusterIndex == -1)
    {
        ...
        for (auto it = trackers.begin(); it != trackers.end();)
        {
            if (it != trackers.end() && (*it).m_time_since_update > max_age) {
                it = trackers.erase(it);     
                //std::cout << "byee tracker nocluster!" << std::endl;
            }
            else {
                Rect_<float> pBox = (*it).predict();
                predictedBoxes.push_back(pBox);
    ...
    for (auto it = trackers.begin(); it != trackers.end();)
    {
        ...
        // remove dead trackers that haven't been updated in 5 frames
            ...
            std::cout << "byee tracker normal!" << std::endl;
            it = trackers.erase(it);         
        }
        else {
            it++;
    ...
    for (int i = 0; i < frameTrackingResult.size(); i++)
    {
        ...
        s.azimuth = 180 * atan2(-xDiff, yDiff) / 3.14159;
        s.elevation = s.z;
        s.power = 0;
        sources.src.push_back(s);
    }
    sources.count = frameTrackingResult.size();
    ...
    std::cout << ros::Time::now().toSec() - performanceTime << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    { // ros::ok()??
        ...
        double begin2 = ros::Time::now().toSec();
        ...
        {
            //13 ms
            process(pcl_points, pcl_pointsDown);
        }
